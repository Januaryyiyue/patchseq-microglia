...
## LOAD DATA ----
h_ephys <- read.csv("/external/rprshnas01/netdata_kcni/stlab/Public/AIBS_patchseq_2020/human/ephys/processed/human_ephys_features_berg.csv")
h_ephys <- dplyr::rename(h_ephys, ephys_session_id = X)

meta <- read.csv("/external/rprshnas01/netdata_kcni/stlab/Public/AIBS_patchseq_2020/human/20200625_patchseq_metadata_human.csv")
meta$transcriptomics_sample_id = sub("-",".", meta$transcriptomics_sample_id)
meta$transcriptomics_sample_id = sub("-",".", meta$transcriptomics_sample_id)
meta$transcriptomics_sample_id = sub("-",".", meta$transcriptomics_sample_id)

micro <- read.xlsx(here("h_microglia_rank.xlsx", sheetName = 'h_microglia_rank'))

# merge
temp <- inner_join(meta, micro, by = "transcriptomics_sample_id")
mm <- dplyr::select(temp, cell_specimen_id, ephys_session_id, transcriptomics_sample_id, Micro_C1QC)

final <- inner_join(mm, h_ephys, by = "ephys_session_id")
...
microglianess <- final$Micro_C1QC
adapt <- final$adapt
...
p9 <- ggplot(
  final,
  aes(x = threshold_v, y = Micro_C1QC)) +
  geom_smooth(method = "lm", se = F) + 
  geom_point() +
...
p12 <- ggplot(
  final,
  aes(x = fast_trough_v, y = Micro_C1QC)) +
  geom_smooth(method = "lm", se = F) + 
  geom_point() +
...
p15 <- ggplot(
  final,
  aes(x = upstroke_downstroke_ratio, y = Micro_C1QC)) +
  geom_smooth(method = "lm", se = F) + 
  geom_point() +
...
p17 <- ggplot(
  final,
  aes(x = fast_trough_t, y = Micro_C1QC)) +
  geom_smooth(method = "lm", se = F) + 
  geom_point() +
...
p19 <- ggplot(
  final,
  aes(x = slow_trough_t, y = Micro_C1QC)) +
  geom_smooth(method = "lm", se = F) + 
  geom_point() +
...
p20 <- ggplot(
  final,
  aes(x = rheo_first_isi, y = Micro_C1QC)) +
  geom_smooth(method = "lm", se = F) + 
  geom_point() +
...
p23 <- ggplot(
  final,
  aes(x = fi_fit_slope, y = Micro_C1QC)) +
  geom_smooth(method = "lm", se = F) + 
  geom_point() +
...
p26 <- ggplot(
  final,
  aes(x = input_resistance, y = Micro_C1QC)) +
  geom_smooth(method = "lm", se = F) + 
  geom_point() +
